...
@section('title', 'Ø§Ù„Ø£Ø¯ÙˆØ§Ø±')
@section('content')

<div class="flex justify-between items-center mb-5">
    <h2 class="text-xl font-bold text-slate-800">Ø§Ù„Ø£Ø¯ÙˆØ§Ø±</h2>
    ...
    <table class="w-full text-sm">
        <thead class="bg-slate-50 text-slate-500 text-xs border-b">
            <tr>
                ...
                <th class="px-4 py-3 text-right">Ø§Ù„Ø±Ù…Ø²</th>
                <th class="px-4 py-3 text-right">Ø§Ù„ØµÙ„Ø§Ø­ÙŠØ§Øª</th>
        ...
        <tbody class="divide-y divide-slate-100">
            ...
            <tr class="hover:bg-slate-50">
                ...
                <td class="px-4 py-3 text-xs text-slate-500">{{ $role->permissions_count ?? $role->permissions->count() }} ØµÙ„Ø§Ø­ÙŠØ©</td>
                <td class="px-4 py-3">
                    <div class="flex gap-2">
                        ...
                        <form action="{{ route('admin.settings.roles.destroy', $role->id) }}" method="POST" class="inline"
                              onsubmit="return confirm('Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ø¯ÙˆØ±ØŸ')">
                            @csrf @method('DELETE')
            ...
            @empty
            <tr><td colspan="5" class="px-4 py-8 text-center text-slate-400">Ù„Ø§ ØªÙˆØ¬Ø¯ Ø£Ø¯ÙˆØ§Ø±</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
