    ...
    <form action="{{ route('admin.contracts.update', $contract->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">
                        ...
                        <button type="button" @click="clientModal.open=true"
                                class="text-blue-500 hover:text-blue-700 mr-1 font-normal text-xs underline">+ إضافة عميل جديد</button>
                    </label>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">محطة القدوم (جنسية العاملة)</label>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <h3 class="text-sm font-bold text-slate-700 mb-5 pb-3 border-b border-slate-100 flex items-center gap-2">
                ...
                التواريخ
                <span class="text-xs font-normal text-slate-400 mr-1">— تاريخ نهاية التجربة والعقد يُحسبان تلقائيًا من تاريخ الوصول</span>
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">
                        تاريخ نهاية التجربة
                        <span class="text-sky-400 font-normal text-xs">+3 شهور</span>
                    </label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">
                        تاريخ نهاية العقد
                        <span class="text-sky-400 font-normal text-xs">+سنتان</span>
                    </label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">
                        ...
                        <button type="button" @click="workerModal.open=true"
                                class="text-indigo-500 hover:text-indigo-700 mr-1 font-normal text-xs underline">+ إضافة عاملة</button>
                    </label>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 mb-1">
                                ...
                                @if($st['days'])<span class="text-xs text-orange-500 bg-orange-50 rounded px-1.5 py-0.5">⏱ {{ $st['days'] }} أيام متوقعة</span>@endif
    ...
    <div x-show="clientModal.open" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden"
            ...
            <div class="bg-blue-600 px-6 py-4 flex items-center justify-between">
                <h3 class="text-white font-bold text-base">إضافة عميل جديد</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" x-model="clientModal.name" placeholder="اسم العميل"
                           class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 transition">
                </div>
                <div>
            ...
            <div class="px-6 pb-5 flex gap-3 justify-end">
                ...
                <button type="button" @click="submitClient()" :disabled="clientModal.loading"
                    ...
                    <span x-show="!clientModal.loading">حفظ العميل</span>
...
function contractForm() {
    return {
        ...
        async submitClient() {
            if (!this.clientModal.name.trim()) { this.clientModal.error = 'الاسم مطلوب'; return; }
        ...
        async submitWorker() {
            if (!this.workerModal.name.trim()) { this.workerModal.error = 'الاسم مطلوب'; return; }
