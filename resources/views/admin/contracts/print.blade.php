...
<head>
    ...
    <style>
        * { font-family: 'Cairo', sans-serif !important; }
        ...
        .timeline-dot-cur   { background: #2563eb; }
...
<body class="bg-slate-100 min-h-screen py-8 px-4">
    ...
    <div class="page max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden" style="padding: 0;">
        ...
        {{-- ── Header ────────────────────────────────────────────────────────── --}}
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%); padding: 28px 36px;">
            <div class="flex items-center justify-between">
                <div class="text-white">
                    <p class="text-xs opacity-70 mb-0.5">نظام الامتياز للاستقدام</p>
                    <h1 class="text-2xl font-extrabold tracking-wide">عقد استقدام</h1>
                    <p class="text-sm opacity-80 mt-1 font-mono">{{ $contract->contract_number }}</p>
                </div>
                <div class="text-left text-white opacity-80 text-sm">
                    <p>تاريخ الطلب</p>
                    <p class="font-bold text-base text-white opacity-100">{{ $contract->request_date?->format('Y/m/d') ?? '—' }}</p>
                    <p class="mt-2">الفرع: <span class="font-bold">{{ $contract->branch?->name ?? '—' }}</span></p>
                </div>
            </div>
        </div>
        ...
        @php
            ...
            $sc = $statusColors[$contract->current_status] ?? ['bg'=>'#dbeafe','txt'=>'#1d4ed8','brd'=>'#93c5fd'];
            ...
            <div style="margin-bottom:20px;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:0; border:1px solid #e2e8f0; border-radius:10px; overflow:hidden;">
                    ...
                    @php
                        ...
                        $txtColor = $isDone ? '#15803d' : ($isCur ? '#1d4ed8' : '#64748b');
                        $dotColor = $isDone ? '#16a34a' : ($isCur ? '#2563eb' : '#e2e8f0');
                    @endphp
                    <div style="display:flex; align-items:center; gap:10px; padding:9px 14px; background:{{ $bgColor }}; border-bottom:1px solid #f1f5f9; {{ $loop->odd ? 'border-left:1px solid #f1f5f9;' : '' }}">
                        ...
                        <div style="flex:1;">
                            ...
                            @elseif($isCur)
                            <p style="font-size:10px; color:#3b82f6; margin:0;">المرحلة الحالية</p>
                            @endif
                        </div>
                    </div>
                    @endforeach
