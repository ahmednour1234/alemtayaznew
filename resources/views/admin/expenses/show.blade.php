...
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">ت�اصيل المصرو�</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div><p class="text-xs text-slate-400">ال�رع</p><p class="font-medium mt-0.5">{{ $expense->branch?->name }}</p></div>
            <div><p class="text-xs text-slate-400">نوع المصرو�</p><p class="font-medium mt-0.5">{{ $expense->expenseType?->name }}</p></div>
        ...
        <div><p class="text-xs text-slate-400">الوص�</p><p class="mt-0.5">{{ $expense->description }}</p></div>
        @endif
        @if($expense->attachment)
        <div><p class="text-xs text-slate-400">المر�ق</p><a href="{{ Storage::url($expense->attachment) }}" target="_blank" class="text-blue-600 text-sm hover:underline mt-0.5 block">عرض المر�ق</a></div>
        @endif
