...
@section('title', 'ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ù…ØµØ±ÙˆÙ')
@section('content')
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ù…ØµØ±ÙˆÙ</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div><p class="text-xs text-slate-400">Ø§Ù„ÙØ±Ø¹</p><p class="font-medium mt-0.5">{{ $expense->branch?->name }}</p></div>
            <div><p class="text-xs text-slate-400">Ù†ÙˆØ¹ Ø§Ù„Ù…ØµØ±ÙˆÙ</p><p class="font-medium mt-0.5">{{ $expense->expenseType?->name }}</p></div>
            <div><p class="text-xs text-slate-400">Ø§Ù„Ù…Ø¨Ù„Øº</p><p class="font-bold text-red-600 text-lg mt-0.5">{{ number_format($expense->amount, 2) }} Ø±ÙŠØ§Ù„</p></div>
            <div><p class="text-xs text-slate-400">Ø§Ù„ØªØ§Ø±ÙŠØ®</p><p class="font-medium mt-0.5">{{ $expense->date?->format('Y-m-d') }}</p></div>
            @if($expense->approved_at)
            <div><p class="text-xs text-slate-400">ØªØ§Ø±ÙŠØ® Ø§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯</p><p class="font-medium mt-0.5">{{ $expense->approved_at?->format('Y-m-d H:i') }}</p></div>
            <div><p class="text-xs text-slate-400">Ø§Ø¹ØªÙ…Ø¯ Ø¨ÙˆØ§Ø³Ø·Ø©</p><p class="font-medium mt-0.5">{{ $expense->approver?->name }}</p></div>
            @endif
        </div>
        ...
        <div><p class="text-xs text-slate-400">Ø§Ù„ÙˆØµÙ</p><p class="mt-0.5">{{ $expense->description }}</p></div>
        @endif
        @if($expense->attachment)
        <div><p class="text-xs text-slate-400">Ø§Ù„Ù…Ø±ÙÙ‚</p><a href="{{ Storage::url($expense->attachment) }}" target="_blank" class="text-blue-600 text-sm hover:underline mt-0.5 block">Ø¹Ø±Ø¶ Ø§Ù„Ù…Ø±ÙÙ‚</a></div>
        @endif
        ...
        <div class="flex flex-wrap gap-3 pt-4 border-t" x-data="{ rejectModal: false }">
            ...
            <a href="{{ route('admin.expenses.edit', $expense->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-5 py-2 rounded-lg">ØªØ¹Ø¯ÙŠÙ„</a>
            @endif
            <a href="{{ route('admin.expenses.index') }}" class="bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm px-5 py-2 rounded-lg">Ø±Ø¬ÙˆØ¹</a>
