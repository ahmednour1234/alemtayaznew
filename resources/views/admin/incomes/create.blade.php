...
@section('title', 'Ø¥Ø¶Ø§ÙØ© Ø¥ÙŠØ±Ø§Ø¯')
@section('content')
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">Ø¥Ø¶Ø§ÙØ© Ø¥ÙŠØ±Ø§Ø¯ Ø¬Ø¯ÙŠØ¯</h2>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.incomes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Ù†ÙˆØ¹ Ø§Ù„Ø¯Ø®Ù„ <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Ø·Ø±ÙŠÙ‚Ø© Ø§Ù„Ø¯ÙØ¹ <span class="text-red-500">*</span></label>
                    <select name="payment_method" required class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="cash" {{ old('payment_method') === 'cash' ? 'selected' : '' }}>Ù†Ù‚Ø¯</option>
                        <option value="bank_transfer" {{ old('payment_method') === 'bank_transfer' ? 'selected' : '' }}>ØªØ­ÙˆÙŠÙ„ Ø¨Ù†ÙƒÙŠ</option>
                        <option value="check" {{ old('payment_method') === 'check' ? 'selected' : '' }}>Ø´ÙŠÙƒ</option>
                        <option value="other" {{ old('payment_method') === 'other' ? 'selected' : '' }}>Ø£Ø®Ø±Ù‰</option>
                    </select>
                </div>
                <div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.incomes.index') }}" class="bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm px-6 py-2.5 rounded-lg">Ø¥Ù„ØºØ§Ø¡</a>
            </div>
        </form>
    </div>
</div>
