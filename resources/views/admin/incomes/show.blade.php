...
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">ت�اصيل الإيراد</h2>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div><p class="text-xs text-slate-400">ال�رع</p><p class="font-medium mt-0.5">{{ $income->branch?->name }}</p></div>
            ...
            <div>
                <p class="text-xs text-slate-400">طريقة الد�ع</p>
            ...
            <div><p class="text-xs text-slate-400">أضي� بواسطة</p><p class="font-medium mt-0.5">{{ $income->admin?->name }}</p></div>
        ...
        <div><p class="text-xs text-slate-400">الوص�</p><p class="mt-0.5">{{ $income->description }}</p></div>
        @endif
        @if($income->attachment)
        <div>
            <p class="text-xs text-slate-400">المر�ق</p>
            <a href="{{ Storage::url($income->attachment) }}" target="_blank" class="text-blue-600 text-sm hover:underline mt-0.5 block">عرض المر�ق</a>
        </div>
        @endif
