...
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">تفاصيل التحويل</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div><p class="text-xs text-slate-400">من فرع</p><p class="font-medium mt-0.5">{{ $transfer->fromBranch?->name ?? 'خارجي' }}</p></div>
            <div><p class="text-xs text-slate-400">إلى فرع</p><p class="font-medium mt-0.5">{{ $transfer->toBranch?->name ?? 'خارجي' }}</p></div>
        ...
        <div><p class="text-xs text-slate-400">الوصف</p><p class="mt-0.5">{{ $transfer->description }}</p></div>
        @endif
