...
<div class="max-w-lg">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">تعديل: {{ $nationality->name }}</h2>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.nationalities.update', $nationality->id) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1.5">اسم الجنسية <span class="text-red-500">*</span></label>
                <input type="text" name="name" value="{{ old('name', $nationality->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $nationality->code) }}" maxlength="10" placeholder="مثال: BD"
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="active" id="active" value="1" {{ old('active', $nationality->active) ? 'checked' : '' }}
