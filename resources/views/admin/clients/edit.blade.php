...
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">تعديل بيانات العميل</h2>
    </div>
    ...
    <form action="{{ route('admin.clients.update', $client->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="marital_status" required
                        ...
                        <option value="single"   {{ old('marital_status', $client->marital_status) === 'single'   ? 'selected' : '' }}>أعزب</option>
                        <option value="married"  {{ old('marital_status', $client->marital_status) === 'married'  ? 'selected' : '' }}>متزوج</option>
                        <option value="divorced" {{ old('marital_status', $client->marital_status) === 'divorced' ? 'selected' : '' }}>مطلق</option>
                        <option value="widowed"  {{ old('marital_status', $client->marital_status) === 'widowed'  ? 'selected' : '' }}>أرمل</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="classification" required
                            class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="potential" {{ old('classification', $client->classification) === 'potential' ? 'selected' : '' }}>محتمل</option>
                        <option value="confirmed" {{ old('classification', $client->classification) === 'confirmed' ? 'selected' : '' }}>مؤكد</option>
                        <option value="premium"   {{ old('classification', $client->classification) === 'premium'   ? 'selected' : '' }}>مميز</option>
                        <option value="blocked"   {{ old('classification', $client->classification) === 'blocked'   ? 'selected' : '' }}>محظور</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="mb-3">
                        <img src="{{ Storage::disk('public')->url($client->national_id_image) }}"
                             alt="صورة الهوية" class="max-h-40 rounded-lg border border-slate-200">
                        <p class="text-xs text-slate-400 mt-1">الصورة الحالية — ارفع صورة جديدة للاستبدال</p>
                    </div>
                    @endif
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="required_nationality_id"
                        ...
                        <option value="{{ $nat->id }}" {{ old('required_nationality_id', $client->required_nationality_id) == $nat->id ? 'selected' : '' }}>{{ $nat->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">نوع العاملة</label>
                    <input type="text" name="worker_type" value="{{ old('worker_type', $client->worker_type) }}"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">الراتب الشهري</label>
                    <input type="number" name="monthly_salary" value="{{ old('monthly_salary', $client->monthly_salary) }}" step="0.01" min="0"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <textarea name="notes" rows="3"
                      class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none">{{ old('notes', $client->notes) }}</textarea>
        </div>
