...
@section('title', __('workers.view.title'))
@section('content')
@php
...
<div class="w-full space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-xl font-bold text-slate-800">{{ __('workers.view.title') }}</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center gap-4 mb-5 pb-4 border-b border-slate-100">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-lg font-bold text-slate-800 truncate">{{ $worker->name ?: __('workers.view.no_name') }}</h3>
                        <div class="flex flex-wrap items-center gap-2 mt-1">
                            ...
                            <span class="text-sm text-slate-500">{{ $worker->nationality->display_name }}</span>
                            @endif
                            @if($worker->branch)
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-5">
                    @php $rows = [
                        ['label' => __('workers.fields.passport'), 'value' => $worker->passport_number],
                        ['label' => __('workers.fields.profession'),      'value' => $worker->profession_label],
                        ['label' => __('workers.fields.gender'),       'value' => $worker->gender_label],
                        ['label' => __('workers.fields.experience'),      'value' => $worker->experience_label],
                        ['label' => __('workers.fields.religion'),     'value' => $worker->religion],
                        ['label' => __('workers.fields.age'),       'value' => $worker->age ? $worker->age.' ' . __('workers.view.years') : null],
                        ['label' => __('workers.fields.phone'),      'value' => $worker->phone],
                    ] @endphp
                    @foreach($rows as $row)
                    <div>
                    ...
                    <div class="col-span-2 md:col-span-3">
                        <p class="text-xs text-slate-400 mb-0.5">{{ __('workers.fields.notes') }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-5">
                    @php
                        ...
                        $cRows = [
                            ['label' => __('workers.view.musaned'),       'value' => $contract->musaned_number],
                            ['label' => __('workers.view.visa_type'),    'value' => $contract->visa_type === 'domestic' ? __('workers.view.visa_domestic') : ($contract->visa_type === 'rehabilitation' ? __('workers.view.visa_rehab') : '—')],
                            ['label' => __('workers.fields.visa'),    'value' => $contract->visa_number],
                            ['label' => __('workers.view.agent'),          'value' => $contract->agent?->name],
                            ['label' => __('workers.view.pay_status'),      'value' => ['pending' => __('workers.view.pay_pending'), 'partial' => __('workers.view.pay_partial'), 'full' => __('workers.view.pay_full')][$contract->payment_status] ?? '—'],
                            ['label' => __('workers.view.total_cost'),  'value' => $contract->total_cost ? number_format($contract->total_cost, 0) . ' ' . __('workers.view.currency') : null],
                            ['label' => __('workers.view.request_date'),     'value' => $contract->request_date],
                            ['label' => __('workers.view.arrival'), 'value' => $contract->arrival_date],
                        ];
                    @endphp
                    ...
                    {{-- Status badge --}}
                    <div>
                        <p class="text-xs text-slate-400 mb-1">{{ __('workers.view.contract_status') }}</p>
                        @php
                            $sColor = match(true) {
                ...
                <div class="mt-5 pt-4 border-t border-slate-100">
                    <p class="text-xs font-semibold text-slate-500 mb-3">{{ __('workers.view.schedule') }}</p>
                    <div class="flex flex-wrap gap-3">
                        @if($contract->arrival_date)
                        <div class="flex-1 min-w-[140px] bg-blue-50 rounded-xl p-3 text-center">
                            <p class="text-xs text-blue-500 mb-1">{{ __('workers.view.arrival') }}</p>
                        ...
                        <div class="flex-1 min-w-[140px] bg-amber-50 rounded-xl p-3 text-center">
                            <p class="text-xs text-amber-600 mb-1">{{ __('workers.view.trial_end') }}</p>
                        ...
                        <div class="flex-1 min-w-[140px] bg-emerald-50 rounded-xl p-3 text-center">
                            <p class="text-xs text-emerald-600 mb-1">{{ __('workers.view.warranty_end') }}</p>
                ...
                <div class="mt-4 pt-4 border-t border-slate-100">
                    <p class="text-xs text-slate-400 mb-1">{{ __('workers.view.contract_notes') }}</p>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm p-5">
                <h4 class="text-sm font-semibold text-slate-600 mb-3">{{ __('workers.view.cv_file') }}</h4>
                <a href="{{ route('admin.workers.cv', $worker->id) }}" target="_blank"
                   class="flex items-center gap-3 p-3 bg-red-50 hover:bg-red-100 rounded-xl transition-colors">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-red-500">{{ __('workers.view.cv_click') }}</p>
                    </div>
                </a>
            </div>
            @endif
            ...
            <div class="bg-white rounded-xl shadow-sm p-5">
                <h4 class="text-sm font-semibold text-slate-600 mb-3">{{ __('workers.view.passport_img') }}</h4>
                <a href="{{ route('admin.workers.passport', $worker->id) }}" target="_blank" class="block">
                    <img src="{{ route('admin.workers.passport', $worker->id) }}" alt="{{ __('workers.fields.passport_alt') }}" class="w-full h-auto rounded-xl border border-slate-200 object-cover hover:opacity-90 transition">
                </a>
                <p class="text-xs text-slate-400 mt-2 text-center">{{ __('workers.view.click_zoom') }}</p>
            </div>
            @endif
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl p-5">
                ...
                <a href="{{ route('admin.clients.show', $client->id) }}"
                   class="mt-3 inline-flex items-center gap-1 text-xs text-blue-600 hover:underline">
                    {{ __('workers.view.view_client') }}
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                </a>
            </div>
            @else
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 text-center">
                <p class="text-sm text-slate-400">{{ __('workers.view.not_assigned') }}</p>
                @if(! $worker->isBooked())
                <a href="{{ route('admin.workers.assign', $worker->id) }}"
                   class="mt-2 inline-block text-xs text-emerald-600 hover:underline">{{ __('workers.view.assign_now') }}</a>
                @endif
            </div>
            @endif
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5">
                <h4 class="text-sm font-semibold text-slate-600 mb-3">{{ __('workers.view.contract_sum') }}</h4>
                <div class="space-y-2 text-xs">
                    <div class="flex justify-between">
                        <span class="text-slate-400">{{ __('workers.view.contract_no') }}</span>
                        <span class="font-mono font-semibold text-blue-700">{{ $contract->contract_number }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-400">{{ __('workers.view.department') }}</span>
                        <span class="font-medium text-slate-700">
                            {{ ['customer_service' => __('workers.view.dep_cs'), 'accounts' => __('workers.view.dep_accounts'), 'coordination' => __('workers.view.dep_coord')][$contract->current_department] ?? '—' }}
                        </span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-400">{{ __('workers.view.payment') }}</span>
                        <span class="font-medium">{{ ['pending' => __('workers.view.pay_pending'), 'partial' => __('workers.view.pay_partial'), 'full' => __('workers.view.pay_full')][$contract->payment_status] ?? '—' }}</span>
                    </div>
                    @if($contract->total_cost)
                    <div class="flex justify-between">
                        <span class="text-slate-400">{{ __('workers.view.cost') }}</span>
                        <span class="font-bold text-emerald-700">{{ number_format($contract->total_cost, 0) }} {{ __('workers.view.currency') }}</span>
                    </div>
                    @endif
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mt-6">
        <h3 class="text-sm font-bold text-slate-700 mb-5 pb-3 border-b border-slate-100 flex items-center gap-2">
            ...
            {{ __('workers.view.activity') }}
            <span class="text-xs font-normal text-slate-400">{{ __('workers.view.activity_hint') }}</span>
        </h3>
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-sm font-semibold text-slate-700">
                        {{ $log->admin?->name ?? $log->admin_name ?? __('workers.view.deleted_user') }}
                    </span>
                    @if($log->isSystem())
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 text-slate-600 font-medium">{{ __('workers.view.auto') }}</span>
                    @endif
        ...
        @empty
        <p class="text-sm text-slate-400 text-center py-6">{{ __('workers.view.no_logs') }}</p>
        @endforelse
    </div>
</div>
@endsection
