...
@section('title', 'تعديل تأمين طبي')
@section('content')
<div class="flex justify-between items-center mb-6">
    <h2 class="text-xl font-bold text-slate-800">تعديل التأمين الطبي</h2>
    <a href="{{ route('admin.hr.insurances.index') }}" class="text-sm text-slate-500 hover:text-slate-700">رجوع</a>
</div>
<div class="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
    <form method="POST" action="{{ route('admin.hr.insurances.update', $insurance->id) }}">
        @csrf @method('PUT')
